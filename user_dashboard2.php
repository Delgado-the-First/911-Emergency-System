...
<html lang="en">
  ...
  <body x-data="" class="is-header-blur" x-bind="$store.global.documentBody">
                                ...
                                <form class="grid grid-cols-2 gap-8 mt-6" method="post" action="process_add_log.php">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <textarea id="remarks" name="remarks" rows="4" class="form-textarea w-full rounded-lg border border-slate-300 bg-transparent p-2.5 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"></textarea>
                                        </div>
                                    </div>
                    ...
                    <?php foreach ($types as $type): ?>
                        <div class="rounded-lg bg-slate-100 p-4 dark:bg-navy-600">
                            <div class="flex justify-between">
                                ...
                                if ($type == 'Police') {
                                    echo '<svg version="1.1" id="Icons" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="size-5 text-primary" x="0px" y="0px" viewBox="0 0 32 32"  xml:space="preserve">
                                    ...
                                    echo '<svg version="1.1" id="Icons" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 32 32" style="enable-background:new 0 0 32 32;" xml:space="preserve" class="size-5 text-primary" x="0px" y="0px" viewBox="0 0 32 32"  xml:space="preserve">
                                            <style type="text/css">
                                    ...
                                    echo '<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 271.2159 167.2434" enable-background="new 0 0 271.2159 167.2434" xml:space="preserve" class="size-5 text-primary" x="0px" y="0px" viewBox="0 0 32 32"  xml:space="preserve">
                                                <g>
        ...
        <div x-data="{activeTab:'tabPending'}" class="tabs flex flex-col">
            ...
            >
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="is-scrollbar-hidden min-w-full overflow-x-auto">
                            <table class="is-zebra w-full text-left" id="callLogTable2">
                                <thead>
                                    <tr>
                                        <th class="whitespace-nowrap rounded-l-lg bg-slate-200 px-3 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">#</th>
                                        <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Type of Service</th>
                    ...
                    <div class="flex flex-col justify-between space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:px-5">
                        <div class="flex items-center space-x-2 text-xs+">
                            <span>Show</span>
                            <label class="block">
                                <select id="entriesPerPage2" class="form-select rounded-full border border-slate-300 bg-white px-2 py-1 pr-6 hover:border-slate-400">
                        ...
                        <ol id="paginationControls2" class="pagination2">
                        
                        </ol>

                        <div id="tableInfo2" class="text-xs+"></div>
                    </div>
    ...
    // Pagination
            ...
            // Pagination2
            document.addEventListener('DOMContentLoaded', function () {
                const table2 = document.querySelector('#callLogTable2 tbody');
                const entriesPerPage2Select = document.querySelector('#entriesPerPage2');
                const paginationControls2 = document.querySelector('#paginationControls2');
                const tableInfo2 = document.querySelector('#tableInfo2');

                let rows = Array.from(table2.querySelectorAll('tr'));
                let currentPage = 1;
                let rowsPerPage = parseInt(entriesPerPage2Select.value);
                ...
                function renderTable() {
                    ...
                    // Update table info
                    const totalRows = rows.length;
                    tableInfo2.textContent = `${start + 1} - ${Math.min(end, totalRows)} of ${totalRows} entries`;

                    renderPaginationControls2(totalRows);
                }

                function renderPaginationControls2(totalRows) {
                    paginationControls2.innerHTML = '';
                    const totalPages = Math.ceil(totalRows / rowsPerPage);
                    ...
                    for (let i = 1; i <= totalPages; i++) {
                        ...
                        paginationControls2.appendChild(li);
                    }
                }

                entriesPerPage2Select.addEventListener('change', () => {
                    rowsPerPage = parseInt(entriesPerPage2Select.value);
                    currentPage = 1; // Reset to the first page
                    renderTable();
                });
